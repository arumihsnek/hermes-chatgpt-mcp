...
### 5.3 Scope model (V4 target contract)

The adapter scope model is the **V4 target contract**. It does not assert that every current live tool has a proven scope mapping; the final catalog is authoritative for current exposure/status/scope distinctions.
...
- [CONTROL_PLANE_SPEC.md](CONTROL_PLANE_SPEC.md) (companion document)
- [CURRENT_STATE.md](CURRENT_STATE.md) (synthesis evidence)
